...
let executereg (thestack: stack_t) (oper: char) (reg: int) =
  try match oper with
    | 'l' -> 
      ...
      else printf "dc: register '%c' (0%o) is empty\n%!"
        (char_of_int reg) reg
    | 's' ->
    ...
    in  try let left = pop thestack
      in  if (operc = '/')
        && ((Bigint.cmp right Bigint.zero) = 0)
          then (eprintf "dc: divide by zero\n%!";
            push left thestack;
            push right thestack)
...
let execute (thestack: stack_t) (oper: char) =
  try match oper with
    ...
    | 'l'  -> failwith "dc: operator l scanned with no register"
    | 'p'  -> print_number (Stack.top thestack)
    | 's'  -> failwith "dc: operator s scanned with no register"
    | '\n' -> ()
    | ' '  -> ()
    | _    -> printf "dc: 0%o is unimplemented\n%!" (ord oper)
  with Stack.Empty -> print_stackempty()
